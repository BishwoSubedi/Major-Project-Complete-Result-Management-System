...
exports.postRelevantStudent = async (req, res, next) => {
  ...
  if (department === 'None') {
    ...
    const sql ='SELECT * FROM student WHERE dept_id = ? GROUP BY s_id';
    const results = await queryParamPromise(sql, [department]);
    res.render('Admin/Student/getStudent', {
      data: results,
      page_name: 'students',
...
exports.postStudentSettings = async (req, res, next) => {
  ...
  const password = rollNumber;
  const hashedPassword = await hashing(password,8);
  const sql2 =
    'UPDATE STUDENT SET s_name = ?, rollNumber=?,gender = ?, dob = ?,email = ?, s_address = ?, contact = ?, password = ?, dept_id = ? WHERE email = ?';
  await queryParamPromise(sql2, [
    name,
    rollNumber,
    gender,
...
exports.postClassSettings = async (req, res, next) => {
  const { staff, course, classId } = req.body;
  const sql =
    'UPDATE class SET st_id = ?, c_id = ? WHERE class_id = ?';
  await queryParamPromise(sql, [staff, course,classId]);
...
exports.postAddCourse = async (req, res, next) => {
  let { course, semester, department, credits} = req.body;
...
exports.postCourseSettings = async (req, res, next) => {
  let { course, semester, department, credits,  courseId } = req.body;
  const sql =
