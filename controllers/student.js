...
// Controller Function for Displaying Semester Result

exports.displaySemesterResult = async (req, res, next) => {
  const { semester } = req.body;

  try {
    // Fetch the results for the chosen semester
    const sql = `
      SELECT course.c_name, result.gpa
      FROM result
      INNER JOIN course ON result.c_id = course.c_id
      WHERE result.s_id = ? AND result.semester = ?;
    `;

    const results = await queryParamPromise(sql, [req.user, semester]);

    // Calculate CGPA
    const numberOfCourses = results.length;
    let totalGPA = 0;
    ...
    const calculatedCGPA = (totalGPA / numberOfCourses).toFixed(2);
